...
public class TextFieldFormatter {
	...
	public void formatter(JTextField tf2, String CaracteresValidos, String mask) {
		try {
			mf.setMask(mask);
		...
		String text = tf2.getText().replaceAll("[\\W]", "");
		...
		/**
		 * if (!text.equals("")) { for (int i = 0; tf.getText().charAt(i) != ' ' && i <
		 * tf.getLength() - 1; i++) { tf.forward(); } }
		 **/
	}
	...
	/**
	 * @param txt the tf to set
	 */
	public void setTf(JTextField txt) {
		this.tf = txt;
	}
